...
class Game {
    ...
    //Velocidad de caída de la pieza
    static PIECE_SPEED = 300;
// Tiempo de animación cuando se borra una fila    static DELETE_ROW_ANIMATION = 500;
    ...
    showWelcome() {
        Swal.fire("Bienvenido", `juego de Tetris en JavaScript.
<br>
...
`);
    ...
    playerLoses() {
// Comprueba si hay algo en Y 1. Quizás no sea justo para el jugador, pero funciona
    ...
    mainLoop() {
        ...
        if (this.figureCanMoveDown()) {
            ...
            // Si la cifra no puede, iniciamos un tiempo de espera porque
           // el jugador puede mover la figura para mantenerla baja
          // por ejemplo cuando la figura se colapsa con otros puntos pero quedan
            // espacio a la izquierda o derecha y el jugador se mueve allí para que la figura pueda seguir bajando
            ...
            setTimeout(() => {
                ...
                // (porque el jugador lo movió) luego regresamos y mantenemos el bucle
                if (this.figureCanMoveDown()) {
                    return;
                }
                // En este punto, sabemos que la figura colapsó con el suelo
                // o con otro punto. Entonces movemos toda la figura a la matriz de piezas existente.
    ...
    cleanGameBoardAndOverlapExistingPieces() {
        for (let y = 0; y < Game.ROWS; y++) {
            for (let x = 0; x < Game.COLUMNS; x++) {
                ...
                // Superponga la pieza existente, si la hubiera
    ...
    /**
     *
     * @param point Un objeto que tiene propiedades x  y; las coordenadas no deben ser globales, sino relativas al punto
     * @returns {boolean}
     */
...
// Devuelve verdadero incluso si el punto no es válido (por ejemplo, si está fuera del límite, porque no es responsabilidad de la función)
    ...
    /**
      * Compruebe si un punto (en el tablero de juego) es válido para poner otro punto allí.
    * @param point el punto a comprobar, con coordenadas relativas
     * @param points una matriz de puntos que conforma una figura
     */
